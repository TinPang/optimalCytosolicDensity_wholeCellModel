...
ttolerance_solver = 1e-12;  # tolerance of solver
...
for jj_repeat in range(Nrepeat):
    while True:
        ...
        def f_get_KM(xx):
            ...
            while True:
                ...
                for qq in range(Nrxn):
                   termm_denominator[qq] = (1+ttheta*factor_ttheta[qq]) * np.exp(log_Gamma[qq])*f_sp_exp2(avg_V,pair_radius[qq][0],pair_radius[qq][1]);
                   termm_numerator[qq] = np.exp(log_Gamma[qq])+ttheta*factor_ttheta[qq]*f_sp_exp2(avg_V,pair_radius[qq][0],pair_radius[qq][1]);
                ...
                for qq in range(Nrxn):
                    if (np.isinf(termm_denominator[qq])):
                        ...
                        cur_KM[qq] = KM0[qq] / tmp_exp / (1+ttheta*factor_ttheta[qq])
                    else:
                        cur_KM[qq] = tmp_KM[qq];
        ...
        res = minimize(Obj_rule, x0, method='SLSQP', jac=False, constraints=cons, options={'ftol': ttolerance_solver, 'maxiter' : 1000, 'disp' : False},bounds=bnds, tol=ttolerance_solver);
